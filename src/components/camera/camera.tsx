...
const Camera = ({ onUpload }: { onUpload: (url: string) => void }) => {
	...
	return (
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent className='p-0 w-screen h-screen'>
				<div className='flex relative flex-col gap-4 justify-center items-center w-screen h-screen'>
					...
					<div className='grid grid-cols-3 gap-4 place-items-center w-full'>
						<div />
						<div
							className='p-2 rounded-full border-2 border-foreground'
							onClick={() => {
								setIsLoading(true)
								capture()
							}}
						>
							<div className='w-12 h-12 rounded-full border-2 border-foreground bg-foreground' />
						</div>
						<div
							className='p-2 rounded-full border-2 border-foreground'
							onClick={handleClick}
