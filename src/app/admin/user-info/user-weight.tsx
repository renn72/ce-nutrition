...
const BodyWeightChart = ({ logs }: { logs: Log[] }) => {
	...
	return (
		<ChartContainer
			config={chartConfig}
			className='w-full opacity-80 min-h-[70px] max-h-[100px]'
		>
			<LineChart data={logs}>
...
const WeightArrow = ({
	...
	return (
		<>
			{weight > lastWeight ? (
				<ArrowUp className='text-secondary-foreground' size={20} />
			) : (
				<ArrowDown className='text-accent-foreground' size={20} />
			)}
		</>
	)
}
...
const UserWeight = ({
	...
	return (
		<div
			className={cn(
				'border rounded-lg p-4 flex flex-col w-full items-center',
				className,
			)}
		>
			<div
			...
			>
				<div className='flex items-baseline'>
					...
					<WeightArrow weight={weight} lastWeight={lastWeight} />
				</div>
			</div>
