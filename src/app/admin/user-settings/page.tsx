...
const Partner = ({ currentUser }: { currentUser: GetUserById }) => {
  const ctx = api.useUtils()
  const { data: users, isLoading } = api.user.getAllYour.useQuery()
  const { mutate: setPartner, isPending } = api.user.setPartner.useMutation({
    onSuccess: async () => {
      toast.success('Partner updated')
      await Promise.all([
        ctx.user.invalidate(),
        ctx.user.getAllYour.invalidate(),
      ])
    },
    onError: (error) => {
    ...
    users
      ?.filter(
        (user) =>
          ...
          (user.isActive || user.id === currentUser.partner?.id),
      )
  ...
  return (
    <div className='flex flex-col gap-3 rounded-lg border p-3 shadow-sm'>
      <div className='space-y-0.5'>
        <Label>Partner</Label>
        <div className='text-sm text-muted-foreground'>
          Link a partner account so their shopping lists can be merged into a
          shared active list.
        </div>
      </div>
      <Select
