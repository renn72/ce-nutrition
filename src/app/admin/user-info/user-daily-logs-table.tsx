...
const TableHeadWrapper = ({ children }: { children: React.ReactNode }) => {
	return (
		<TableHead className='text-xs tracking-normal lg:tracking-tighter xl:text-sm xl:tracking-normal'>
			{children}
		</TableHead>
	)
}
...
const TableCellWrapper = ({
	...
	return (
		<TableCell
			className={cn(
				className,
				'text-xs tracking-normal lg:tracking-tighter xl:text-sm xl:tracking-normal',
			)}
		>
			{children}
		</TableCell>
	...
	return (
		<div className={cn('border rounded-lg overflow-y-scroll', className)}>
			<Table>
				<TableHeader>
					<TableRow>
						<TableHeadWrapper>Date</TableHeadWrapper>
						<TableHeadWrapper>Weight</TableHeadWrapper>
						{columnsToShow.waistMeasurement && (
							<TableHeadWrapper>Waist</TableHeadWrapper>
						)}
						<TableHeadWrapper>Calories</TableHeadWrapper>
						<TableHeadWrapper>Protein</TableHeadWrapper>
						{columnsToShow.sleep && <TableHeadWrapper>Sleep</TableHeadWrapper>}
						{columnsToShow.sleepQuality && (
							<TableHeadWrapper>Sleep Quality</TableHeadWrapper>
						)}
						{columnsToShow.nap && <TableHeadWrapper>Nap</TableHeadWrapper>}
						{columnsToShow.steps && <TableHeadWrapper>Steps</TableHeadWrapper>}
						{columnsToShow.cardio && (
							<TableHeadWrapper>Cardio</TableHeadWrapper>
						)}
						{columnsToShow.cardioType && (
							<TableHeadWrapper>Cardio Type</TableHeadWrapper>
						)}
						{columnsToShow.liss && <TableHeadWrapper>LISS</TableHeadWrapper>}
						{columnsToShow.mobility && (
							<TableHeadWrapper>Mobility</TableHeadWrapper>
						)}
						{columnsToShow.hiit && <TableHeadWrapper>HIIT</TableHeadWrapper>}
						{columnsToShow.weightTraining && (
							<TableHeadWrapper>Weight Training</TableHeadWrapper>
						)}
						{columnsToShow.posing && (
							<TableHeadWrapper>Posing</TableHeadWrapper>
						)}
						{columnsToShow.sauna && <TableHeadWrapper>Sauna</TableHeadWrapper>}
						{columnsToShow.coldPlunge && (
							<TableHeadWrapper>Cold Plunge</TableHeadWrapper>
						)}
						{columnsToShow.fastedBloodGlucose && (
							<TableHeadWrapper>Fasted BG</TableHeadWrapper>
						)}
						{columnsToShow.notes && <TableHeadWrapper>Notes</TableHeadWrapper>}
					</TableRow>
				</TableHeader>
				<TableBody>
					{dailyLogs
						...
						.map((log) => {
							...
							return (
								...
								>
									...
									<TableCellWrapper>
										{mealsMacros?.cals.toFixed(0)}
									</TableCellWrapper>
									<TableCellWrapper>
										{mealsMacros?.protein.toFixed(1)}
									</TableCellWrapper>
									{columnsToShow.sleep && (
										<TableCellWrapper>{log.sleep}</TableCellWrapper>
									)}
									{columnsToShow.sleepQuality && (
									...
									{columnsToShow.nap && (
										<TableCellWrapper>
											{Number(log.nap).toFixed(1)}
										</TableCellWrapper>
									)}
									{columnsToShow.steps && (
										<TableCellWrapper>{log.steps}</TableCellWrapper>
									)}
									{columnsToShow.cardio && (
										<TableCellWrapper>{log.cardio}</TableCellWrapper>
									)}
									{columnsToShow.cardioType && (
										<TableCellWrapper>{log.cardioType}</TableCellWrapper>
									)}
									{columnsToShow.liss && (
										<TableCellWrapper>{log.liss}</TableCellWrapper>
									)}
									{columnsToShow.mobility && (
										<TableCellWrapper>{log.mobility}</TableCellWrapper>
									)}
									{columnsToShow.hiit && (
										<TableCellWrapper>{log.hiit}</TableCellWrapper>
									)}
									{columnsToShow.weightTraining && (
										<TableCellWrapper>{log.weight}</TableCellWrapper>
									)}
									{columnsToShow.posing && (
										<TableCellWrapper>{log.posing}</TableCellWrapper>
									)}
