...
export const testRouter = createTRPCRouter({
  ...
  importAFCDLiquid: protectedProcedure.mutation(async ({ ctx }) => {
    const file = readFileSync(
      './src/server/data/ingredient-liquid-test.csv',
      'utf8',
    )
    ...
    // @ts-ignore
    const ins = csvData.map((row) => {
      return ctx.db.insert(ingredient).values({
        userId: user?.id ?? '',
        serveSize: '100',
        serveUnit: 'ml',
    ...
    const res = await ctx.db.batch(ins)
  ...
  importAFCDSolid: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    for (const row of csvData) {
      await ctx.db.transaction(async (tx) => {
        ...
        tx.insert(ingredientAdditionOne).values({
          ingredientId: res[0]?.id ?? 0,
          moisture: row['Moisture (water) \n(g)'],
          nitrogen: row['Nitrogen \n(g)'],
          alcohol: row['Alcohol \n(g)'],
          fructose: row['Fructose \n(g)'],
        ...
        tx.insert(ingredientAdditionTwo).values({
          ingredientId: res[0]?.id ?? 0,
          aceticAcid: row['Acetic acid \n(g)'],
          citricAcid: row['Citric acid \n(g)'],
          fumaricAcid: row['Fumaric acid \n(g)'],
        ...
        tx.insert(ingredientAdditionThree).values({
          ingredientId: res[0]?.id ?? 0,
