...
export function calculateMealMacros(ingredients: any[] | undefined) {
	...
	return ingredients.reduce(
		(acc, curr) => {
			...
			const serveSize = Number(curr?.ingredient?.serveSize) || 100
			const scale = size / serveSize

			const cals = Number(curr?.ingredient?.caloriesWFibre) || 0
			const calsWO = Number(curr?.ingredient?.caloriesWOFibre) || 0
			const protein = Number(curr?.ingredient?.protein) || 0
			const carbs =
				Number(curr?.ingredient?.availableCarbohydrateWithSugarAlcohols) || 0
			const fat = Number(curr?.ingredient?.fatTotal) || 0

			return {
				calsWFibre: acc.calsWFibre + cals * scale,
				cals: acc.cals + calsWO * scale,
				protein: acc.protein + protein * scale,
				carbs: acc.carbs + carbs * scale,
				fat: acc.fat + fat * scale,
			}
		},
		{ cals: 0, protein: 0, carbs: 0, fat: 0, calsWFibre: 0 },
	)
}
...
export function getRecipeDetailsFromDailyLog(
	...
	const ingredients = dailyLog?.dailyMeals.find(
		(meal) => meal.mealIndex === mealIndex,
	)?.ingredients
...
export function getMealMacro(userPlan: UserPlan, mealIndex: number) {
	const recipes = userPlan?.userRecipes.filter(
	...
	const ingredients = userPlan?.userIngredients.filter(
		(ingredient) =>
...
export function getMealCals(userPlan: UserPlan, mealIndex: number) {
	const recipes = userPlan?.userRecipes.filter(
		(recipe) => recipe.mealIndex === mealIndex,
	)

	if (recipes?.length !== 1)
		return Number(userPlan?.userMeals[mealIndex]?.calories) || 0
...
export function getRecipeDetailsForDailyLog(
	...
	const recipeIndex = userPlan?.userRecipes.find(
		(recipe) => recipe.id === recipeId,
	)?.recipeIndex
	const mealIndex = userPlan?.userRecipes.find(
		(recipe) => recipe.id === recipeId,
	)?.mealIndex
...
export function getRecipeDetailsForUserPlanNew(
	...
	const carbs = recipe?.ingredients.reduce((acc, curr, i) => {
		const cal = Number(curr?.ingredient?.availableCarbohydrateWithSugarAlcohols)
		const size = ingredientsSize[i] || 100
		const scale = size / Number(curr?.ingredient?.serveSize)
		return acc + cal * scale
	}, 0)
