...
const Log = memo(
	({
		...
		const waterAmount = useMemo(
			() =>
				todaysDailyLog?.waterLogs?.reduce(
					(acc, curr) => acc + Number(curr.amount),
					0,
				),
			[todaysDailyLog?.waterLogs],
		)

		const yesterdaysPoopCount = useMemo(
			() => yesterdaysDailyLog?.poopLogs?.length || 0,
			[yesterdaysDailyLog?.poopLogs],
		)
		const yesterdaysWaterAmount = useMemo(
			() =>
				yesterdaysDailyLog?.waterLogs?.reduce(
					(acc, curr) => acc + Number(curr.amount),
					0,
				),
