...
const OutOfBoundsDay = ({ day }: OutOfBoundsDayProps) => (
	<div className='relative p-1 w-full h-full text-xs bg-secondary text-muted-foreground'>
		{day}
	</div>
)
...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
	...
	for (let day = 1; day <= daysInMonth; day++) {
		...
		days.push(
			<div
				className='flex relative flex-col gap-1 p-1 w-full h-full text-xs text-muted-foreground'
				key={day}
			>
				{day}
				<div>
					{featuresForDay.slice(0, 7).map((feature) => children({ feature }))}
				</div>
				{featuresForDay.length > 7 && (
...
export const CalendarDate = ({ children }: CalendarDateProps) => (
	<div className='flex justify-between items-center p-3'>{children}</div>
)
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
	...
	return (
		<div className={cn('grid flex-grow grid-cols-7', className)}>
			{daysData.map((day) => (
				<div className='p-3 text-xs text-right text-muted-foreground' key={day}>
					{day}
				</div>
			))}
		</div>
